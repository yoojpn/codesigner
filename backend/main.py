...
async def run_agent(user_message: str, history: list, ws: WebSocket, session_dir: Path, chat_id: str, thinking_level: str = "none"):
    ...
    cmd = [
        ...
        "--system-prompt", (
            ...
            "6. 【重要】大きなファイル（500行超）を読む場合は必ずGrepやGlobで該当箇所を絞り込んでから部分的に読むこと。ファイル全体をReadしてはならない。コンテキストウィンドウが溢れる。\n"
            "7. 【重要】挨拶・雑談・簡単な質問（「こんにちは」「ありがとう」「調子どう？」など）にはツールを一切使わず、そのまま日本語で返答すること。コード作業の依頼があって初めてツールを使うこと。\n"
            "8. 【最重要】タスクは必ず最後まで完遂すること。途中で止まったり、説明だけして実装を省略したり、「続きは次のメッセージで」などと分割してはならない。ファイルの編集・作成は一度のツール呼び出しで完全な状態にすること。"
        ),
    ]
    if session_id:
